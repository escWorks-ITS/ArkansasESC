...
public partial class catalog_register_confirmuserprofile : region4.escWeb.BasePage
{
    ...
    protected void btnSaveUserProfileClick(object sender, EventArgs e)
    {
       ...
       // ((escWeb.tx_r19.ObjectModel.User)CurrentUser).SpecialNeeds_Id = ddlSpecialNeeds.SelectedValue == "" ? 0 : Convert.ToInt32(ddlSpecialNeeds.SelectedValue);
        CurrentUser.Save();

        this.ShoppingCart.ChangeUser(CurrentUser);
        //string isGroup = escWorks.Tools.Strings.CleanUp("isGroup", true, false);

        if (this.ShoppingCart.HasConference)
        {
            if (CurrentUser.isCampusRegister || CurrentUser.isDistrictRegister)
                Session["isGroup"] = "1";
            else
            {
