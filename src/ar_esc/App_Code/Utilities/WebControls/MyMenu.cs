...
namespace region4.escWorks.WebControls
{
    public class MenuV3 : WebControl
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            //string appPath = region4.escWeb.SiteVariables.CustomerSiteId;// HttpContext.Current.Request.ApplicationPath;
            ...
            customerId = region4.escWeb.SiteVariables.CustomerSiteId;
            signInUrl = "~/lib/img/" + customerId + "/buttons/signin.gif";
            signOutUrl = "~/lib/img/" + customerId + "/buttons/signout.gif";
        ...
        {
            if (region4.escWeb.SiteVariables.CustomerSiteId == "ar_ade")
            {
                if (item.ItemID == 113 || item.ItemID == 36)
                    item.Visible = false;
            }
            ...
            if (item.Display(Context)) //Passes permissions test here
            {
                ...
                if (item.Href.StartsWith("http://") || item.Href.StartsWith("www") || item.Href.StartsWith("javascript") || item.Href.StartsWith("https://dev-") || item.Href.StartsWith("https://uat-") || item.Href.StartsWith("https://wwww") || item.Href.StartsWith("https://"))
                {
                    if (item.Href.Contains("cid"))
                    {
                        item.Href = (ConfigurationManager.AppSettings["database.activeserver"] == "production") ? item.Href : (item.Href.StartsWith("https://") ? "https://" + ConfigurationManager.AppSettings["database.activeserver"] + "-" + item.Href.Substring(item.Href.IndexOf("www", 0)) : "http://" + ConfigurationManager.AppSettings["database.activeserver"] + "-" + item.Href.Substring(item.Href.IndexOf("www", 0)));
                    ...
                    if (item.ItemID == 222)
                        writer.Write(String.Format("<li{5}><a{6} href=\"{0}{1}\"  title=\"{2}\" target=\"{3}\">{4}</a>", "", item.Href, item.ToolTip, item.Target, item.Name, itemCss, anchorCss));
                    else
            ...
            //    if (!String.IsNullOrEmpty(item.AnchorCssClass))
            //        anchorCss = String.Format(" class=\"{0}{1}\"", item.AnchorCssClass, false ? " menuItemSelected" : "");
            //    if (!String.IsNullOrEmpty(item.ItemCssClass))
            //        itemCss = String.Format(" class=\"{0}\"", item.ItemCssClass);

            //    if (item.Href.StartsWith("http://") || item.Href.StartsWith("www") || item.Href.StartsWith("javascript") || item.Href.StartsWith("https://dev-") || item.Href.StartsWith("https://uat-"))
            //        writer.Write(String.Format("<li{5}><a{6} href=\"{1}\" title=\"{2}\" target=\"{3}\">{4}</a>", currentPage.PathToRoot, item.Href, item.ToolTip, item.Target, item.Name, itemCss, anchorCss));
            //    else if (String.IsNullOrEmpty(item.Href))
            //        writer.Write(String.Format("<li{5}><a{6}  title=\"{2}\" target=\"{3}\">{4}</a>", currentPage.PathToRoot, item.Href, item.ToolTip, item.Target, item.Name, itemCss, anchorCss));
