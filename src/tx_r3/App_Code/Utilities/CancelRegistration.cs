    ...
    public class CancelRegistration : region4.Utilities.Financial.CancelRegistration
    {
        ...
        public override void ProcessPORefund(DataSet DS, int Payment_id, region4.ObjectModel.Attendee attendee)
        {
            ...
            using (SqlConnection conn = region4.Common.DataConnection.DbConnection)
            {
                ...
                for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                           // PORefundAmount = Convert.ToDouble(Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2)) - Math.Max(Convert.ToDouble(Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2)) * .25, Convert.ToDouble(ConfigurationManager.AppSettings["session.ammount.applyTorefund"]));
                        PORefundAmount = Convert.ToDouble(Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2)) - Convert.ToDouble(Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2)) * .25;
                    }
                    ...
                    if (PORefundAmount > 0.0)
                        //SQLCommand.Parameters.AddWithValue("@cancellation_fee", Math.Max(Convert.ToDouble(Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2)) * .25, Convert.ToDouble(ConfigurationManager.AppSettings["session.ammount.applyTorefund"])));
                        SQLCommand.Parameters.AddWithValue("@cancellation_fee", Convert.ToDouble(Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2)) * .25);
                    else
        ...
        public override void ProcessCCRefund(DataSet DS, region4.ObjectModel.Attendee attendee)
        {
            ...
            for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
            {
                ...
                else
                {
                     ...
                     //   RefundTransaction.Amount = Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2) - Math.Max(Convert.ToDecimal(Convert.ToDouble(drv["amount"].ToString()) * .25), Convert.ToDecimal(ConfigurationManager.AppSettings["session.ammount.applyTorefund"]));
                    RefundTransaction.Amount = Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2) - Convert.ToDecimal(Convert.ToDouble(drv["amount"].ToString()) * .25);
                }
                //added by Z
        ...
        private void RecordTransactionData(region4.Utilities.Financial.CreditCardTransaction transaction, string transaction_id, string amount, region4.ObjectModel.Attendee attendee)
        {
            ...
            using (SqlConnection conn = region4.Common.DataConnection.DbConnection)
            {
                ...
                if (transaction.Amount > 0)
                {
                       ...
                       // SQLCommand.Parameters.AddWithValue("@cancellation_fee", Math.Max(Convert.ToDouble(Decimal.Round(Convert.ToDecimal(amount), 2)) * .25, Convert.ToDouble(ConfigurationManager.AppSettings["session.ammount.applyTorefund"])));
                    SQLCommand.Parameters.AddWithValue("@cancellation_fee", Convert.ToDouble(Decimal.Round(Convert.ToDecimal(amount), 2)) * .25);
                }
                else
        ...
        public override void ProcessDebit(DataSet DS, region4.ObjectModel.Attendee attendee)
        {
            ...
            for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
            {
                ...
                else
                {
                   ...
                   // eRefundTransaction.Amount = Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2) - Math.Max(Convert.ToDecimal(Convert.ToDouble(drv["amount"].ToString()) * .25), Convert.ToDecimal(ConfigurationManager.AppSettings["session.ammount.applyTorefund"]));
                    eRefundTransaction.Amount = Decimal.Round(Convert.ToDecimal(drv["amount"].ToString()), 2) - Convert.ToDecimal(Convert.ToDouble(drv["amount"].ToString()) * .25);
                }
        ...
        private void RecordACHTransactionData(region4.Utilities.Financial.ElectronicCheckTransaction transaction, string transaction_id, string cancelfee, region4.ObjectModel.Attendee attendee)
        {
            using (SqlConnection conn = region4.Common.DataConnection.DbConnection)
            {
                ...
                if (transaction.Amount > 0)
                   // SQLCommand.Parameters.AddWithValue("@cancellation_fee", Math.Max(Convert.ToDouble(Decimal.Round(Convert.ToDecimal(cancelfee), 2)) * .25, Convert.ToDouble(ConfigurationManager.AppSettings["session.ammount.applyTorefund"])));
                    SQLCommand.Parameters.AddWithValue("@cancellation_fee", Convert.ToDouble(Decimal.Round(Convert.ToDecimal(cancelfee), 2)) * .25);
                else
