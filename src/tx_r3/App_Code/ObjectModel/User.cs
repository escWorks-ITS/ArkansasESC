...
namespace escWeb.tx_r3.ObjectModel
{
    ...
    public class User : region4.ObjectModel.User
    {
        ...
        private string _specialNeedIds;
        public string SpecialNeedIds
        {
            get { return _specialNeedIds; }
            set { _specialNeedIds = value; }
        }

        private string _specialNeedsComments;
        public string SpecialNeedsComments
        {
            get { return _specialNeedsComments; }
            set { _specialNeedsComments = value; }
        }
        ...
        protected override void LoadCustomerInfo(System.Data.SqlClient.SqlCommand cmd)
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                if (reader.Read())
                {
                    ...
                    _specialNeedIds = reader["specialNeedIds"].ToString();
                    _specialNeedsComments = reader["specialNeedsComments"].ToString();
                }
                reader.Close();
            }
            cmd.CommandText = "";
        ...
        protected override void SaveCustomerInfo(System.Data.SqlClient.SqlCommand cmd)
        {
            ...
            cmd.Parameters.AddWithValue("@special_need_ids", SpecialNeedIds);
            cmd.Parameters.AddWithValue("@special_need_comments", SpecialNeedsComments);
            ...
            if (SpecialNeedIds != "")
            {//delete special user.multi.specialneeds
                cmd.CommandText = "[p.objectModel.User.multi.specialneeds.delete]";
                cmd.CommandType = System.Data.CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@sid", this.Sid);
                cmd.ExecuteNonQuery();
                ...
                //insert special user.multi.specialneeds
                foreach (string itm in SpecialNeedIds.Split(','))
                {
                    cmd.CommandText = "[p.objectModel.User.multi.specialneeds.insert]";
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@sid", this.Sid);
                    cmd.Parameters.AddWithValue("@item_id", Convert.ToInt32(itm));
                    cmd.ExecuteNonQuery();
