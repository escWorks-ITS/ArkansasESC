...
namespace tx_r16.Shoebox.Evaluation
{
    ...
    {
        private string _Title, _Description,_sTitle;
        ...
        public Evaluation(string Title, string Description, bool Completed,string ShortTitle)
        {
            _Title = Title;
            ...
            using (SqlCommand SQLCommand = new SqlCommand(region4.baseStoredProcedures.LegacyCode.LoadEvaluation, Connection))//this._DataProvider.GenerateConnection()))
            {
                ...
                try
                {
                    ...
                    using (SqlDataReader Dr = SQLCommand.ExecuteReader(CommandBehavior.CloseConnection))
                    {
                        if (Dr.Read())
                        {
                            this._Title = "<font size =5> Region III Education Service Center</font><br>" + Dr["title"].ToString();
                            this._sTitle = Dr["title"].ToString();
