...
public partial class Search : System.Web.UI.UserControl
{
    ...
    {
        if (!chkFF.Checked && !chkVideo.Checked && !chkNet3.Checked)
        {
            lbError.Text = "You must select either Face-to-Face or Online or NET3.";
    ...
    private void LoadData()
    {
        ...
        grdSearch.DataSource = SearchInMemory(sessionInfoList, chkFF.Checked, chkVideo.Checked, chkNet3.Checked, chkFree.Checked, chkWeekend.Checked, keywords);
        grdSearch.DataBind();
    }
    ...
    protected List<SessionInfo> SearchInMemory(List<SessionInfo> sessionInfoList, bool faceToFace, bool online, bool net3, bool free, bool weekend, string Keywords)
    {
        ...
        foreach (SessionInfo sessionInfo in sessionInfoList)
        {
            ...
            if (weekend && (sessionInfo.IsFaceToFace || sessionInfo.IsConference || sessionInfo.IsMultiVenue || sessionInfo.IsNet3Event || sessionInfo.IsNet3StudentEvent || sessionInfo.IsZoomEvent))
                OKtoAdd &= ( 
                            ...
                            || 
                            (sessionInfo.StartDate.DayOfWeek == DayOfWeek.Sunday) 
                            || 
                            (sessionInfo.StartDate.Hour >= 17)
